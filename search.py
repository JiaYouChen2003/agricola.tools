...
class SearchMachine():
    ...
    def getCardRank(self, card_name, game_type = '4player_default'):
        ...
        arr_rank = self.getValuesFromSheet(workbook_list[game_type])
        card_rank = self.getItemIndexByNameFromArray(card_name, arr_rank, 1, 0)
        return card_rank

    def getCardDiff(self, card_name):
        workbook_list = self.workbook_list

        arr_diff = self.getValuesFromSheet(workbook_list['Diff'])
        card_diff = self.getItemIndexByNameFromArray(card_name, arr_diff, 0, 3)
        return card_diff
